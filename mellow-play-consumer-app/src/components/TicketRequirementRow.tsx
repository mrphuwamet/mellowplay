        ...
        return (
          <div key={i} className="flex items-center justify-between gap-2 px-2.5 py-1.5 bg-slate-50 rounded-lg">
            ...
            <div className="flex items-center gap-1.5 shrink-0">
              <span className={`flex items-center gap-1 text-[11px] font-black ${insufficient ? 'text-red-500' : 'text-emerald-600'}`}>
                <Ticket size={11} />
                {balance}
              </span>
              {insufficient && (
                <button
                  ...
                  className="flex items-center gap-0.5 text-[10px] font-black text-white bg-mellow-purple px-1.5 py-0.5 rounded-md active:scale-95 transition-transform"
                >
                  <Plus size={10} />
                  {lang === 'en' ? 'Buy' : 'ซื้อ'}
                </button>
